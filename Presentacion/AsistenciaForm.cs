...
namespace Presentacion
{
    public partial class AsistenciaForm : Form
    {
        ...
        public AsistenciaForm(IServiceProvider serviceProvider, ControlReservas formularioPrincipal)
        {
            InitializeComponent();
        ...
        private void ListarAsistencias()
        {
            try
            {
                var asistencias = _serviceProvider.GetService<IServicioAsistencia>().ObtenerAsistencias();
        ...
        private void btnAgregar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                _serviceProvider.GetService<IServicioAsistencia>().AgregarAsistencia(asistencia);

                MessageBox.Show("Asistencia creada con éxito");
        ...
        private void dgvAsistencias_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (e.RowIndex >= 0)
                {
                    if (dgvAsistencias.Columns[e.ColumnIndex].Name == "Eliminar")
                    {
                        int id = Convert.ToInt32(dgvAsistencias.Rows[e.RowIndex].Cells["Id"].Value);

                        _serviceProvider.GetService<IServicioAsistencia>().EliminarAsistencia(id);

                        MessageBox.Show("Asistencia eliminada con éxito");
                        ListarAsistencias();
                    }
                    else if (dgvAsistencias.Columns[e.ColumnIndex].Name == "Editar")
                    {
                        idAsistencia = Convert.ToInt32(dgvAsistencias.Rows[e.RowIndex].Cells["Id"].Value);
                        dtpFecha.Value = Convert.ToDateTime(dgvAsistencias.Rows[e.RowIndex].Cells["Fecha"].Value);
                        cmbMiembro.SelectedValue = Convert.ToInt32(dgvAsistencias.Rows[e.RowIndex].Cells["IdMiembro"].Value);
                        cmbClase.SelectedValue = Convert.ToInt32(dgvAsistencias.Rows[e.RowIndex].Cells["IdClase"].Value);
                        cmbEntrenador.SelectedValue = Convert.ToInt32(dgvAsistencias.Rows[e.RowIndex].Cells["IdEntrenador"].Value);
                        idMiembro = (int)cmbMiembro.SelectedValue;
                        idClase = (int)cmbClase.SelectedValue;
                        idEntrenador = (int)cmbEntrenador.SelectedValue;
        ...
        private void btnActualizar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                _serviceProvider.GetService<IServicioAsistencia>().ActualizarAsistencia(asistencia);
