...
{
    public partial class ReportesAsistenciasForm : Form
    {
        ...
        public ReportesAsistenciasForm(IServiceProvider serviceProvider, ControlReservas formularioPrincipal)
        {
            InitializeComponent();
        ...
        private void btnFiltrar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var asistencias = _serviceProvider.GetService<IServicioAsistencia>().ObtenerAsistenciasPorMiembro(idMiembro);

                    dgvReportes.DataSource = null;
                    dgvReportes.DataSource = asistencias;

                    idMiembro = 0;
                }
                else if (tipoReporte.Equals("Clases") && idClase != 0)
                {
                    var asistencias = _serviceProvider.GetService<IServicioAsistencia>().ObtenerAsistenciasPorClase(idClase);
                ...
                {
                    var asistencias = _serviceProvider.GetService<IServicioAsistencia>().ObtenerAsistenciasPorEntrenador(idEntrenador);
